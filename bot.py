...
def generate_data_for_night():
    ...
    forecast_tokyo = livedoor.get_today_weather(url, tokyo)
    forecast_osaka = livedoor.get_today_weather(url, osaka)
    forecast_nagoya = livedoor.get_today_weather(url, nagoya)
    forecast_sapporo = livedoor.get_today_weather(url, sapporo)
...
def tweet_wrapper(pattern):
    ...
    print(pcmax.post_tweet(tweet_url, tweet_input, tweet_text).status_code)
    print('/*--- tweet end ---*/')
    print('')
